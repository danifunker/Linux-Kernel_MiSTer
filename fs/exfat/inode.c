...
static int exfat_fill_inode(struct inode *inode, struct exfat_dir_entry *info)
{
	...
	} else if (info->attr & ATTR_SYSTEM) { /* symbolic link */
		/*
		 * MiSTer: a regular file carrying the SYSTEM attribute is a
		 * symbolic link whose target path is stored as the file body.
		 * Using the standard SYSTEM attribute means the link survives
		 * being copied on Windows/macOS, where normal copy tools
