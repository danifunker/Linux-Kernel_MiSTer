...
static int __exfat_unlink(struct inode *dir, struct inode *inode)
{
	struct exfat_chain cdir;
	struct exfat_dentry *ep;
	struct super_block *sb = dir->i_sb;
...
/*
 * MiSTer: read a symbolic link.  The target path is stored verbatim as the
 * file body (no NUL terminator, as written by exfat_symlink() and by the
 * previous out-of-tree driver), so read exactly i_size bytes and terminate.
 */
...
/*
 * MiSTer: create a symbolic link as an exFAT file flagged with the SYSTEM
 * attribute whose body holds the target path.  See exfat_fill_inode().
 */
static int exfat_symlink(struct user_namespace *mnt_userns, struct inode *dir,
			 struct dentry *dentry, const char *target)
{
	...
	if (len == 0)
		return -ENOENT;
	/* the target path is stored within a single page */
	if (len > PAGE_SIZE)
		return -ENAMETOOLONG;

	mutex_lock(&EXFAT_SB(sb)->s_lock);
	exfat_set_volume_dirty(sb);
	err = exfat_add_entry(dir, dentry->d_name.name, &cdir, TYPE_SYMLINK,
			      &info);
	exfat_clear_volume_dirty(sb);
	if (err) {
		mutex_unlock(&EXFAT_SB(sb)->s_lock);
	...
	err = PTR_ERR_OR_ZERO(inode);
	/*
	 * Release s_lock before writing the link body: the page write path
	 * (exfat_get_block) acquires s_lock itself.
	 */
	mutex_unlock(&EXFAT_SB(sb)->s_lock);
	if (err)
		return err;

	/* store the target path as the file body */
	err = page_symlink(inode, target, len);
	if (err) {
		/* roll back the directory entry created above */
		__exfat_unlink(dir, inode);
		iput(inode);
		return err;
	}
