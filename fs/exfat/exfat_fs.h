...
/*
 * MiSTer: creation-only pseudo type for a symbolic link.  On disk a symlink
 * is an ordinary file (EXFAT_FILE) carrying the standard SYSTEM attribute,
 * with the target path stored as the file body.  It is never returned by
 * exfat_get_entry_type() (which reports it as TYPE_FILE); the SYSTEM bit is
 * what marks it as a link on read.
 */
#define TYPE_SYMLINK		0x011E
